...
export default function FamilyTreeAdvanced() {
    ...
    if (svgRef.current && treeData && zoomRef.current) {
      try {
        ...
        if (fullWidth > 0 && fullHeight > 0) {
          // حساب scale للتمركز
          const scale = Math.min((width * 0.8) / fullWidth, (height * 0.8) / fullHeight, 1.0);
  ...
  const exportTreeAsImage = useCallback(async () => {
    ...
    try {
      ...
      gClone.setAttribute(
        'transform',
        `translate(${-bounds.x + padding}, ${-bounds.y + padding + titleHeight})`
      );
  ...
  const buildTreeFromRelations = useCallback((persons) => {
    ...
    const normalizeArabic = (str) => {
      if (!str) return '';
      return str.trim().replace(/أ|إ|آ/g, 'ا').replace(/ة/g, 'ه').replace(/ى/g, 'ي').toLowerCase();
    };
    ...
    const buildNode = (person) => {
      ...
      return {
        ...
        spouse: personWives[0]
          ...
          : null,
        children: children.map((child) => buildNode(child)).filter((node) => node !== null), // إزالة العقد الفارغة (المكررة)
      };
    };
          ...
          response.persons.map((p) => {
            return [
              p.id,
              {
                ...
                relation:
                  p.relation || (p.is_root ? 'رب العائلة' : p.gender === 'M' ? 'ابن' : 'بنت'),
  ...
  const drawTreeWithD3 = useCallback((data) => {
    ...
    if (!isLargeTree) {
      links
        .on('mouseenter', function () {
          d3.select(this).style('stroke-width', 4).style('opacity', 1).style('stroke', '#1976d2');
        })
        .on('mouseleave', function () {
          d3.select(this).style('stroke-width', 3).style('opacity', 0.9).style('stroke', '#2196f3');
        });
    }
    ...
    const drawPersonCard = (nodeGroup, nodeData, offsetX = 0, isSpouseCard = false) => {
      ...
      const name =
        nodeData.name || `${nodeData.firstName || ''} ${nodeData.fatherName || ''}`.trim() || '';
      ...
      const textCenterX =
        cardStartX + padding + avatarSize + (cardWidth - padding - avatarSize) / 2;
    ...
    nodes.each(function (d) {
      ...
      if (
        d.children &&
        d.children.length > 0 &&
        !nodeData.isVirtualRoot &&
        !nodeData.isVirtualGrandfather
      ) {
        ...
        if (hasGrandchildren) {
          childText = isMobile
            ? `${d.children.length}`
            : ` ${d.children.length}/${grandchildrenCount}`;
      ...
      if (
        ...
        name.toLowerCase().includes(searchQueryRef.current.toLowerCase())
      ) {
        nodeGroup
          .select('rect.family-node-card')
          .transition()
          .duration(600)
          .attr('stroke', '#f59e0b')
          .attr('stroke-width', 3);
        ...
        spousesData.forEach((spouseData, spouseIndex) => {
          ...
          const linkStartX =
            spouseIndex === 0
              ? cardWidth / 2
              : baseSpouseOffset +
                (spouseIndex - 1) * (cardWidth + (isMobile ? 10 : 15)) +
                cardWidth / 2;
          nodeGroup
          ...
          spouseGroup
            ...
            .on('mouseenter', function (event) {
              ...
              d3.select(this)
                ...
                .style(
                  'transform',
                  `translate(${spouseOffset}px, 0) scale(1.05) translate(${-spouseOffset}px, 0)`
                )
      ...
      nodeGroup
        ...
        .on('mouseenter', function (event) {
          ...
          d3.selectAll('.link')
            .filter(
              (linkData) =>
                linkData.source.data.id === d.data.id || linkData.target.data.id === d.data.id
            )
            .style('stroke', '#1976d2')
          ...
          if (isCurrentlyCollapsed) {
            ...
            descendantIds.forEach((descId) => {
              ...
              g.selectAll('.link')
                .filter((linkData) => {
                  const sourceId =
                    linkData.source?.data?.attributes?.id || linkData.source?.data?.id;
                  const targetId =
                    linkData.target?.data?.attributes?.id || linkData.target?.data?.id;
            ...
            // تغيير مظهر الزر
            collapseGroup.select('circle').transition().duration(200).attr('fill', '#4caf50');
            collapseGroup.select('text').text('−');
            collapseGroup
              .selectAll('text')
              .filter((_, i) => i === 1)
              .remove();
            ...
            descendantIds.forEach((descId) => {
              ...
              const nodesToHide = g.selectAll('.node').filter(function () {
                const nodeData = d3.select(this).datum()?.data;
                const id = nodeData?.attributes?.id || nodeData?.id;
                return id === descId;
              });
              ...
              nodesToHide.transition().duration(300).style('opacity', 0);

              // إخفاء كامل بعد انتهاء الـ transition باستخدام setTimeout (iOS compatible)
              setTimeout(() => {
                nodesToHide.style('visibility', 'hidden').style('display', 'none');
              }, 320);

              // إخفاء الروابط
              const linksToHide = g.selectAll('.link').filter((linkData) => {
              ...
              linksToHide.transition().duration(300).style('opacity', 0);

              // إخفاء كامل بعد انتهاء الـ transition
              setTimeout(() => {
                linksToHide.style('visibility', 'hidden').style('display', 'none');
              }, 320);
            });

            // تغيير مظهر الزر
            collapseGroup.select('circle').transition().duration(200).attr('fill', '#ff9800');
            collapseGroup.select('text').text('+');
  ...
  const performSearch = useCallback((query) => {
    if (!query || query.trim().length < 2) {
      ...
      if (svgRef.current) {
        const svg = d3.select(svgRef.current);
        svg
          .selectAll('.node rect.family-node-card')
          .transition()
          .duration(300)
          .style('stroke', null)
              ...
              >
                {loading ? (
                  <Box textAlign="center" maxWidth={600}>
                    ...
                    <Typography
                      variant="body2"
                      sx={{ color: '#10b981', fontFamily: 'Cairo, sans-serif' }}
                    >
                  ...
                  <Box textAlign="center">
                    ...
                    <Typography
                      variant="h4"
                      sx={{ mb: 1, fontFamily: 'Cairo, sans-serif', color: '#10b981' }}
                    >
                      🌳 ابنِ شجرة قبيلتك
                    </Typography>
                    <Typography
  ...
  const renderToolbar = () => (
    <Paper
      ...
      sx={{
        ...
        zIndex: 1000,
        background:
          'linear-gradient(145deg, rgba(255,255,255,0.98) 0%, rgba(249,250,251,0.98) 50%, rgba(243,244,246,0.98) 100%)',
  ...
  return (
    <Box
      className="family-tree-advanced-root"
      sx={{ width: '100vw', height: '100vh', fontFamily: 'Cairo, sans-serif' }}
    >
      {renderToolbar()}
      <Box
        sx={{
              ...
              if (collapsedNodes.size > 0) {
                ...
                if (treeData) {
                  ...
                  nodesToHide.forEach((hideId) => {
                    const nodesToHideElements = g.selectAll('.node').filter(function () {
                      const nodeData = d3.select(this).datum()?.data;
                      const id = nodeData?.attributes?.id || nodeData?.id;
                      return id === hideId;
                    });
                    ...
                    // ثم الـ transition
                    nodesToHideElements.transition().duration(400).style('opacity', 0);

                    // إخفاء كامل بعد الـ transition باستخدام setTimeout (iOS compatible)
                    setTimeout(() => {
                      nodesToHideElements.style('visibility', 'hidden').style('display', 'none');
                    }, 420);

                    const linksToHideElements = g.selectAll('.link').filter((linkData) => {
                      const sourceId =
                        linkData.source?.data?.attributes?.id || linkData.source?.data?.id;
                      const targetId =
                        linkData.target?.data?.attributes?.id || linkData.target?.data?.id;
                    ...
                    linksToHideElements.transition().duration(400).style('opacity', 0);

                    setTimeout(() => {
                      linksToHideElements.style('visibility', 'hidden').style('display', 'none');
                    }, 420);
                  });
      ...
      <Dialog open={!!selectedNode} onClose={() => setSelectedNode(null)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ color: '#1976d2', fontWeight: 'bold', fontFamily: 'Cairo, sans-serif' }}>
          ...
          (selectedNode?.relation && FEMALE_RELATIONS.includes(selectedNode?.relation))
            ? '♀️'
            : '♂️'}{' '}
          {selectedNode?.name || 'تفاصيل الشخص'}
        </DialogTitle>
        <DialogContent>
          {selectedNode && (
            <Box sx={{ p: 1 }}>
              ...
              <Box sx={{ mb: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {selectedNode.id === membership?.person_id && (
                  <Chip label="أنا (رب العائلة)" color="success" variant="outlined" />
                )}
                  ...
                  {(() => {
                    ...
                    if (grandchildrenCount > 0) {
                      return (
                        <Typography
                          variant="body2"
                          sx={{ mb: 1, color: '#2196f3', fontWeight: 'bold' }}
                        >
